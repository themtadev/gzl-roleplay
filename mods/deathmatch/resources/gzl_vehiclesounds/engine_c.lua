ENGINE_ENABLED = true
ENGINE_VOLUME_MASTER = 0.25
ENGINE_VOLUME_THROTTLE_BOOST = 2.5
ENGINE_SOUND_FADE_DIMENSION = 6969
		...
		if isElement(vehicle) then
			...
			if engine then
				...
				if getVehicleEngineState(vehicle) == true and distance < ENGINE_SOUND_DISTANCE*2 then
					...
					data.forceNeutral = isSkidding
								or (isLineOfSightClear(x, y, z, x, y, z-(getElementDistanceFromCentreOfMassToBaseOfModel(vehicle)*1.25), true, false, false, true, true, false, false, vehicle) and data.throttle)
								or isElementFrozen(vehicle) or isElementInWater(vehicle)
								or (( rx > 110 ) and ( rx < 250 ))
					...
					if data.changingGear then
						local progress = (now-data.changingTargetRPM.time) / 300
					...
					if data.wasRevLimited then
						...
						if changedGear == "up" and math.random(1, 4) == 1 then
							if data.als then
								data.activeALS = true
							end
