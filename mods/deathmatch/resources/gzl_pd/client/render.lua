...
function PDRender.draw()
    ...
    if PDClient.panicAlert then
        ...
        if (now - PDClient.panicAlert.startTick) < 15000 then
            ...
            dxDrawRectangle(boxX, boxY, boxW, boxH, tocolor(220, 38, 38, alpha))
            dxDrawText("⚠ 10-99 ACİL DESTEK ÇAĞRISI ⚠", boxX, boxY + 8, boxX + boxW, boxY + 28, tocolor(255, 255, 255, 255), 1.0, "default-bold", "center", "center")
            dxDrawText("Memur: " .. PDClient.panicAlert.officerName .. " (GPS İşaretlendi)", boxX, boxY + 30, boxX + boxW, boxY + 54, tocolor(255, 255, 255, 255), 0.9, "default-bold", "center", "center")
        else
    ...
    dxDrawRectangle(t.x - 2, t.y - 2, t.w + 4, t.h + 4, colors.tabletBorder)
    dxDrawRectangle(t.x, t.y, t.w, t.h, colors.tabletBg)

    local hd = t.header
    dxDrawRectangle(hd.x, hd.y, hd.w, hd.h, colors.headerBg)
    dxDrawRectangle(hd.x, hd.y + hd.h - 1, hd.w, 1, colors.separator)

    dxDrawText("★ LOS SANTOS POLICE DEPARTMENT - MDC V2.0", hd.x + 20, hd.y, hd.x + 400, hd.y + hd.h, colors.textPrimary, fontScale * 1.05, "default-bold", "left", "center")
    ...
    dxDrawText(offInfo, hd.x + hd.w - 300, hd.y, hd.x + hd.w - 50, hd.y + hd.h, offStatusCol, fontScale * 0.95, "default-bold", "right", "center")
    ...
    dxDrawText("✕", hd.x + hd.w - 36, hd.y + 16, hd.x + hd.w - 12, hd.y + 40, closeCol, fontScale * 1.1, "default-bold", "center", "center")

    local sb = t.sidebar
    dxDrawRectangle(sb.x, sb.y, sb.w, sb.h, colors.sidebarBg)
    dxDrawRectangle(sb.x + sb.w - 1, sb.y, 1, sb.h, colors.separator)
    ...
    dxDrawRectangle(sb.x + 14, panicY, sb.w - 28, 44, panicBg)
    dxDrawText("🚨 10-99 PANİK BUTONU", sb.x + 14, panicY, sb.x + sb.w - 14, panicY + 44, colors.textPrimary, fontScale * 0.9, "default-bold", "center", "center")
    ...
    local ct = t.content
    dxDrawRectangle(ct.x, ct.y, ct.w, ct.h, colors.contentBg)
