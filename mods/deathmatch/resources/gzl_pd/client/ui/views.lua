...
function Views.drawDashboard(cx, cy, cw, ch, state, fontScale)
    ...
    dxDrawText("TELSİZ & GÖREV DURUMU", cx + 16, curY, cx + 300, curY + 20, colors.textDim, fontScale * 0.85, "default-bold", "left", "center")
    curY = curY + 24
    ...
    dxDrawText("AKTİF DEVRİYE BİRLİKLERİ", cx + 16, curY, cx + 16 + halfW, curY + 20, colors.textDim, fontScale * 0.85, "default-bold", "left", "center")
    dxDrawRectangle(cx + 16, curY + 24, halfW, listH, colors.cardBg)
    dxSetScissor(cx + 16, curY + 24, halfW, listH)

    if not state.officers or #state.officers == 0 then
        dxDrawText("Aktif devriye memuru yok.", cx + 16, curY + 60, cx + 16 + halfW, curY + 100, colors.textDim, fontScale, "default", "center", "center")
    else
        local offY = curY + 28
        for _, off in ipairs(state.officers) do
            ...
            dxDrawText(off.name, cx + 28, offY, cx + 200, offY + 24, colors.textPrimary, fontScale * 0.9, "default-bold", "left", "center")
            dxDrawText(off.status, cx + halfW - 70, offY, cx + halfW + 4, offY + 24, statusCol, fontScale * 0.85, "default-bold", "center", "center")
            dxDrawRectangle(cx + 28, offY + 25, halfW - 24, 1, colors.separator)
            offY = offY + 30
        end
    end
    dxSetScissor()
    ...
    dxDrawText("GENEL ASAYİŞ BÜLTENİ (APB)", rightX, curY, rightX + halfW, curY + 20, colors.textDim, fontScale * 0.85, "default-bold", "left", "center")
    dxDrawRectangle(rightX, curY + 24, halfW, listH, colors.cardBg)
    dxSetScissor(rightX, curY + 24, halfW, listH)

    if not state.bulletins or #state.bulletins == 0 then
        dxDrawText("Yayınlanmış aktif bülten bulunmuyor.", rightX, curY + 60, rightX + halfW, curY + 100, colors.textDim, fontScale, "default", "center", "center")
    else
        local bY = curY + 28
        for _, b in ipairs(state.bulletins) do
            dxDrawText(b.title, rightX + 16, bY, rightX + halfW - 16, bY + 20, colors.policeGold, fontScale * 0.95, "default-bold", "left", "center")
            dxDrawText(b.description, rightX + 16, bY + 22, rightX + halfW - 16, bY + 54, colors.textSecondary, fontScale * 0.85, "default", "left", "top", true, true)
            dxDrawRectangle(rightX + 16, bY + 58, halfW - 32, 1, colors.separator)
            bY = bY + 64
        end
    end
    dxSetScissor()
...
function Views.drawCitizenQuery(cx, cy, cw, ch, state, fontScale)
    ...
    if state.searchedCitizenName and state.searchedCitizenName ~= "" then
        ...
        local cardH = 90
        dxDrawRectangle(cx + 16, curY, cw - 32, cardH, colors.cardBg)
        dxDrawRectangle(cx + 16, curY, 6, cardH, (state.citizenWarrant and colors.accentRed or colors.accentGreen))

        dxDrawText("VATANDAŞ SİCİL DOSYASI: " .. string.upper(state.searchedCitizenName), cx + 32, curY + 12, cx + 500, curY + 36, colors.textPrimary, fontScale * 1.15, "default-bold", "left", "center")
        ...
        dxDrawText(statusStr, cx + 32, curY + 38, cx + cw - 200, curY + 60, statusCol, fontScale * 0.9, "default-bold", "left", "center")
        ...
        dxDrawText("GEÇMİŞ SABIKA VE CEZA KAYITLARI", cx + 16, curY, cx + 400, curY + 20, colors.textDim, fontScale * 0.85, "default-bold", "left", "center")
        curY = curY + 24

        local tableH = ch - (curY - cy) - 16
        dxDrawRectangle(cx + 16, curY, cw - 32, tableH, colors.cardBg)
        dxSetScissor(cx + 16, curY, cw - 32, tableH)

        if not state.citizenRecords or #state.citizenRecords == 0 then
            dxDrawText("Bu vatandaşın adli sicil kaydı temizdir.", cx + 16, curY + 80, cx + cw - 16, curY + 120, colors.textDim, fontScale, "default", "center", "center")
        else
            ...
            for idx, rec in ipairs(state.citizenRecords) do
                ...
                dxDrawRectangle(cx + 24, rowY, cw - 48, 44, (isHover and colors.cardHover or colors.inputBg))

                dxDrawText(rec.crime_title, cx + 36, rowY, cx + 350, rowY + 44, colors.textPrimary, fontScale * 0.9, "default-bold", "left", "center", true)
                dxDrawText("Ceza: $" .. tostring(rec.fine_amount) .. " | Hapis: " .. tostring(rec.jail_time) .. " dk", cx + 360, rowY, cx + 600, rowY + 44, colors.policeGold, fontScale * 0.85, "default", "left", "center")
                dxDrawText("Memur: " .. tostring(rec.officer_name), cx + cw - 220, rowY, cx + cw - 40, rowY + 44, colors.textDim, fontScale * 0.8, "default", "right", "center")
        ...
        dxDrawText("Sorgulama yapmak için yukarıdaki arama kutusuna isim girin.", cx + 16, cy + 180, cx + cw, cy + 220, colors.textDim, fontScale, "default", "center", "center")
    end
end
...
function Views.drawVehicleQuery(cx, cy, cw, ch, state, fontScale)
    ...
    if state.searchedPlate and state.searchedPlate ~= "" then
        local cardH = 80
        dxDrawRectangle(cx + 16, curY, cw - 32, cardH, colors.cardBg)
        dxDrawRectangle(cx + 16, curY, 6, cardH, (state.vehicleBolo and colors.accentRed or colors.accentGreen))

        dxDrawText("PLAKA SORGUSU: " .. string.upper(state.searchedPlate), cx + 32, curY + 12, cx + 500, curY + 36, colors.textPrimary, fontScale * 1.15, "default-bold", "left", "center")
    ...
    dxDrawText("TÜM ARANAN / BOLO LİSTESİ", cx + 16, curY, cx + 400, curY + 20, colors.textDim, fontScale * 0.85, "default-bold", "left", "center")
    curY = curY + 24

    local listH = ch - (curY - cy) - 16
    dxDrawRectangle(cx + 16, curY, cw - 32, listH, colors.cardBg)
    dxSetScissor(cx + 16, curY, cw - 32, listH)

    if not state.allWantedVehicles or #state.allWantedVehicles == 0 then
        dxDrawText("Aktif aranan araç kaydı bulunmuyor.", cx + 16, curY + 80, cx + cw - 16, curY + 120, colors.textDim, fontScale, "default", "center", "center")
    else
        local rowY = curY + 8 - state.scrollOffset
        for idx, bolo in ipairs(state.allWantedVehicles) do
            ...
            dxDrawText("PLAKA: " .. bolo.plate, cx + 36, rowY, cx + 220, rowY + 44, colors.accentRed, fontScale * 0.95, "default-bold", "left", "center")
            dxDrawText("Sebep: " .. bolo.reason, cx + 230, rowY, cx + 550, rowY + 44, colors.textPrimary, fontScale * 0.85, "default", "left", "center", true)
            dxDrawText("Kayıt: " .. bolo.officer_name, cx + cw - 200, rowY, cx + cw - 40, rowY + 44, colors.textDim, fontScale * 0.8, "default", "right", "center")
...
function Views.drawPenalCode(cx, cy, cw, ch, state, fontScale)
    ...
    dxDrawText("SAN ANDREAS CEZA KANUNU VE TARİFELERİ", cx + 16, curY, cx + 500, curY + 24, colors.textPrimary, fontScale * 1.1, "default-bold", "left", "center")
    if state.searchedCitizenName and state.searchedCitizenName ~= "" then
        dxDrawText("Hedef Vatandaş: " .. state.searchedCitizenName, cx + cw - 300, curY, cx + cw - 16, curY + 24, colors.policeGold, fontScale * 0.9, "default-bold", "right", "center")
    end
    curY = curY + 32
    ...
    for idx, penal in ipairs(PDConfig.PenalCode) do
        ...
        dxDrawRectangle(cx + 24, rowY, cw - 48, 54, (isHover and colors.cardHover or colors.inputBg))
        ...
        dxDrawRectangle(cx + 24, rowY, 4, 54, catCol)

        dxDrawText("[" .. penal.category .. "] " .. penal.title, cx + 36, rowY + 6, cx + cw - 260, rowY + 28, colors.textPrimary, fontScale * 0.95, "default-bold", "left", "center")
        dxDrawText("Para Cezası: $" .. tostring(penal.fine) .. " | Kamu Hizmeti / Hapis: " .. tostring(penal.jail) .. " Dakika", cx + 36, rowY + 28, cx + cw - 260, rowY + 48, colors.policeGold, fontScale * 0.85, "default", "left", "center")
