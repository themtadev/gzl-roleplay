...
lib.callback.register('ox_inventory:actinmioc', function(source, invType, data)
...
lib.callback.register('ox_inventory:useItem', function(source, itemName, slot, metadata)
	...
	if inventory.player then
		...
		if item and data and data.count > 0 and data.name == item.name then
			...
			local Player = thombursa.Functions.GetPlayerByCitizenId(inventory.owner)
			...
			if consume and consume ~= 0 and not data.component then
				...
				durability = consume ~= 0 and consume < 1 and data.metadata.durability
...
lib.addCommand({'ver', 'giveitem'}, {
	...
	restricted = 'thombursa.admin',
}, function(source, args)
	if tonumber(args.count) >= 100 then return end
	local src = source
	local item = Items(args.item)

	if item then
		local inventory = Inventory(tonumber(args.target))
		...
		TriggerEvent('jezzy-log-gonder',
			('"%s" verildi %sx %s verilenkişi "%s"'):format(source.label, count, item.name, inventory.label).. " Veren Admin id ve isim: "..src.." "..GetPlayerName(src),
			false,
			''
		)
			lib.logger(source.owner, 'admin', ('"%s" verdi %sx %s verilenkişi "%s"'):format(source.label, count, item.name, inventory.label))
			TriggerEvent('qb-log:server:CreateLog', "admin", "GiveItem", "default", "giveitem " .. source.label .. " " .. item.name .. " " .. count)
...
RegisterServerEvent('envanteractim-sv')
AddEventHandler('envanteractim-sv', function(idamk, acik)
	TriggerClientEvent('envanteractim', idamk, acik)
end)