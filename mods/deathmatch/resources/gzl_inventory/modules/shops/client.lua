...
for shopType, shopData in pairs(data('shops') ) do
	local shop = {
		name = shopData.name,
			...
			elseif shop.targets then
				for i = 1, #shop.targets do
					...
					elseif target.loc then
						...
						local ped = `a_m_m_fatlatin_01`
						if string.match(shopid, "Ballas") or string.match(shopid, "Blackmarket") or string.match(shopid, "Families") then
							ped = `cs_stretch`
						end
						if target.pedaq then
							exports["jezzy-base"]:pedcreate(shopid, ped, target.loc.x, target.loc.y, target.loc.z - 1, target.heading or 0.0)
						end
