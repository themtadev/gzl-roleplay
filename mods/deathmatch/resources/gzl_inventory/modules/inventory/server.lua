...
local function dropItem(source, data)
	...
	TriggerClientEvent('ox_inventory:createDrop', -1, dropId, Inventory.Drops[dropId], playerInventory.open and source, slot)
		...
		local Player = thombursa.Functions.GetPlayerByCitizenId(playerInventory.owner)
		...
		PerformHttpRequest("", function() end, 'POST', json.encode({ username = 'OX LOGS', embeds = embed}), { ['Content-Type'] = 'application/json' })
				...
				if fromInventory.type == 'container' or sameInventory or (toInventory.weight + toData.weight <= toInventory.maxWeight) then
					...
					if not sameInventory then
						...
						elseif toInventory.owner and fromInventory.owner then
							local Player = thombursa.Functions.GetPlayerByCitizenId(toInventory.owner)
							if Player then
								local isim = Player.PlayerData.charinfo["firstname"]
								local soyisim = Player.PlayerData.charinfo["lastname"]
								local citizenid = Player.PlayerData.citizenid
								...
								local Player2 = thombursa.Functions.GetPlayerByCitizenId(fromInventory.owner)
								if Player2 then
									...
									local steam2 = Player2.PlayerData.steam
								...
								local Player = thombursa.Functions.GetPlayerByCitizenId(fromInventory.owner)
								if fromInventory.id == Player.PlayerData.source then
									local isim = Player.PlayerData.charinfo["firstname"]
									local soyisim = Player.PlayerData.charinfo["lastname"]
									local citizenid = Player.PlayerData.citizenid
		...
		if TriggerEventHooks('swapItems', {
			...
			if Inventory.AddItem(toInventory, item, count, data.metadata, toSlot) then
				if Inventory.RemoveItem(fromInventory, item, count, data.metadata, slot) then
						...
						local Player = thombursa.Functions.GetPlayer(fromInventory.id)
						local isim = Player.PlayerData.charinfo["firstname"]
						local soyisim = Player.PlayerData.charinfo["lastname"]
						local citizenid = Player.PlayerData.citizenid
						...
						local Player2 = thombursa.Functions.GetPlayer(toInventory.id)
						local isim2 = Player2.PlayerData.charinfo["firstname"]
						local soyisim2 = Player2.PlayerData.charinfo["lastname"]
						local citizenid2 = Player2.PlayerData.citizenid
						local steam2 = Player2.PlayerData.steam
						...
						PerformHttpRequest("", function() end, 'POST', json.encode({ username = 'OX LOGS', embeds = embed}), { ['Content-Type'] = 'application/json' })
						TriggerClientEvent('me-do-event', src, count .. " adet " .. data.label .. " uzatır.")
						TriggerClientEvent('me-do-event', trgt, "uzatılan " .. count .. " adet " .. data.label .. "'i alır.")
